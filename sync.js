...
(function () {
  ...
  const GOAL_RESET_KEY = 'goals_reset_2026_06_08_v1';
  ...
  function isGoalDataKey(k) {
    return k && (k.indexOf('goals:') === 0 || k === 'goal_streak_v1');
  }

  function hasGoalResetMarker(remote) {
    if (remote && remote[GOAL_RESET_KEY]) return true;
    try { return localStorage.getItem(GOAL_RESET_KEY) === 'true'; } catch (e) { return false; }
  }

  function purgeGoalsOnce() {
    try {
      if (localStorage.getItem(GOAL_RESET_KEY) === 'true') return;
      const keys = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (isGoalDataKey(k)) keys.push(k);
      }
      keys.forEach((k) => localStorage.removeItem(k));
      localStorage.setItem(GOAL_RESET_KEY, 'true');
    ...
    function applyRemote(remote) {
      ...
      try {
        for (const k of Object.keys(remote)) {
          ...
          if (isGoalDataKey(k) && hasGoalResetMarker(remote)) continue;
